 ...
 * @author FANNY BURGOS
 */
public class CorgranosBuilder implements ILiquidacionDePaddyBuilder{
    LiquidacionDePaddy lp;
    boolean bolsa;

    public CorgranosBuilder() {
        lp=null;
    }
    public CorgranosBuilder(LiquidacionDePaddy lp) {
        this.lp = lp;
    }
    @Override
    ...
    void AsignarPesoLiquidado(){        
       double Hum,Imp;
        ...
        lp.setPeso_liquidado(Math.round(lp.getPesodestarado()*Hum*Imp));
         
        
        /*
        double humedad=lp.getHumedad_lab()-lp.getHumedad_pact();
        double impureza=lp.getImpureza_lab()-lp.getImpureza_lab();
        double gobierno=(humedad+impureza)*(0.3);
        double descuentos=humedad+impureza+gobierno;
        System.out.println(descuentos);
        lp.setPeso_liquidado(lp.getPesodestarado()*((100-descuentos)/100));
        */
    }
